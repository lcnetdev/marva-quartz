    ...
    methods: {
        ...
        addDefaultOption(){
          ...
          if (this.bulkOptions.indexOf(o)==-1){
            ...
            this.preferenceStore.setValue('--o-literal-lang-bulk-options',this.bulkOptions )
        ...
        removeDefault(val){
          ...
          this.bulkOptions = this.bulkOptions.filter((v)=>{ return (v !== val)  })
          this.preferenceStore.setValue('--o-literal-lang-bulk-options',this.bulkOptions )
    ...
    >
        ...
        >
          <div id="non-latin-bulk-content" ref="nonLatinBulkContent" @mousedown="onSelectElement($event)" @touchstart="onSelectElement($event)">
            ...
            <div class="view-mode">
              <button :class="{'button-selected':(viewMode=='nonLatin')}" @click="viewMode='nonLatin'">View Non-Latin Literals</button>
            ...
            <details>
              ...
              <div class="value-display-holder" >
                <div>
              ...
              <div>
                <button @click="addDefaultOption()" style="float: right;">Add Lang/Script Option</button>
              </div>
            </div>

            <div class="existing-bulk-options" v-if="bulkOptions.length>0">
              <div>Existing Default Options:</div>
              <div class="existing-bulk-options-option" v-for="o in bulkOptions">
                <span>{{o}}</span><button @click="removeDefault(o)">Remove</button>
              </div>
...
.nonlatin-literal-value{
  font-size: 1.1em;

}

.nonlatin-literal-uri{
  font-size: 0.75em;
  color:rgb(71, 71, 71)
}

.existing-bulk-options-option{
  padding-left: 1em;
}
.existing-bulk-options-option button{
  font-size: 0.75em;
  margin-left: 1em;
}

.existing-bulk-options{
  margin-top: 2em;
