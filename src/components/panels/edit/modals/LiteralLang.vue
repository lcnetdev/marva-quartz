  ...
  export default {
    ...
    data() {
      return {
        ...
        iso639_1: isoLangLib.iso639_1,
        iso639_2: isoLangLib.iso639_2,
        iso15924: isoLangLib.iso15924,
    ...
    computed: {
      ...
      ...mapWritableState(useProfileStore, ['literalLangInfo','literalLangShow']),
    ...
    methods: {
        ...
        /**
        * Takes a string like "@ru-Cryl" and returns it as obj
        * @return {object} - object with lang and script keys
        */  
        returnLangScript(comboString){

          // if they pass a null
          if (!comboString){
            return {lang:null,script:null}
          }
          comboString = comboString.replace("@",'')
          comboString = comboString.split("-")
          if (comboString.length>1){
            return {lang:comboString[0],script:comboString[1]}
          }else{
            return {lang:comboString[0],script:null}
          }
        ...
        setLanguage(event,value){
          let lang = this.$refs[value['@guid'] +'-lang'][0].value
          let script = this.$refs[value['@guid'] +'-script'][0].value
          let langStr = ""
          ...
          if (lang != '' && script != ''){
            ...
            alert('Cannot just set the script value, you must set the langauge first.')
            event.preventDefault()
            this.$refs[value['@guid'] +'-script'][0].value = ""
            return false
          }
  
          // if we are reusing this modal in the createHub or elsewhere, if the guid is set to fake then it will not try to change the value
          if (value['@guid'] === 'fake'){ 
            this.currentlySelectedLang = langStr
            return false
          }



          this.profileStore.setValueLiteral(this.literalLangInfo.componentGuid,value['@guid'],this.literalLangInfo.propertyPath,value.value,langStr)

          for (let sValue of this.literalLangInfo.values){
            if (sValue['@guid'] == value['@guid']){
        ...
        clearLanguage(event,value){

          if (this.literalLangInfo.values[0]['@guid'] == 'fake'){
            this.$emit('langStrSet', null)
            return false
          }

          let langStr = 'REMOVE_COMMAND'

          this.profileStore.setValueLiteral(this.literalLangInfo.componentGuid,value['@guid'],this.literalLangInfo.propertyPath,value.value,langStr)
          for (let sValue of this.literalLangInfo.values){
            if (sValue['@guid'] == value['@guid']){
              sValue['@language']  = null
            }            
          }
    ...
    async mounted() {
      ...
      this.currentlySelectedLang = null
    ...
    >
        ...
        >
          <div id="debug-content" ref="debugContent" @mousedown="onSelectElement($event)" @touchstart="onSelectElement($event)">
            ...
            <div class="value-display-holder" v-for="v in literalLangInfo.values">

              <div class="value-display">{{ v.value }}<span v-if="returnLangScript(v['@language']).lang != null">@{{ v['@language'] }}</span></div>
              ...
              <select @change="setLanguage($event,v)" :ref="v['@guid']+'-'+'script'">
