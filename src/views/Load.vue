            ...
            <div class="load-columns">
              ...
              <div class="load-test-data-column">
                      ...
                      <div style="display:flex">
                        ...
                        <div style="flex:1">
                          ...
                          <span v-if="searchByLccnResults.length == 1" style="display:none;">
                            <label :for="'lccnsearch' + idx">Select</label><input type="radio" v-model="lccnLoadSelected"
                              :value="r" name="lccnToLoad" :id="'lccnsearch' + idx" :name="'lccnsearch' + idx"
                              checked="true" />
                          </span>
                          <span v-else>
                ...
                <div class="load-buttons">
                  <button class="load-button" @click="loadUrl(s.instance)"
                    ...
                    v-for="s in startingPointsFiltered">{{ s.name }}</button>
                </div>
                <hr>
                  ...
                  <ul class="continue-record-list">
                    <li class="continue-record" v-for="record in continueRecords">
                      <router-link :to="{ name: 'Edit', params: { recordId: record.eid } }">
                        <div><span class="continue-record-title">{{ record.title }}</span><span v-if="record.contributor">
                            by
                        ...
                        <div class="continue-record-lastedit"><span v-if="record.status == 'posted'">Posted</span><span
                            v-if="record.status == 'unposted'">last edited</span> <span>{{ returnTimeAgo(record.timestamp)
                            }}</span>
                        </div>
                      </router-link>
              ...
              <div>
                ...
                <div style="padding:5px;">
                  ...
                  Voyager.
                  Marva cannot currently assign Voyager bib numbers, so you need to create a stub record in Voyager and
                  then load
                  it into Marva.
                </div>
                <details>
                  <summary><span style="text-decoration: underline;">Click Here</span> to access blank record templates.
                    Currently
